...
public class Number188 {
    ...
    public int maxProfit(int k, int[] prices) {
        if(prices == null || prices.length < 2)
            return 0;
        int maxTransactions = maxTransactions(prices);
        k = Math.min(k, maxTransactions);
        if(k < 1)
            return 0;
        if(k == 1)
            return maxProfitOnce(prices, 0, prices.length-1);
        if(k == maxTransactions)
            return maxProfitAny(prices);
        int[][][] profits = new int[prices.length][prices.length][k];
        for(int transaction = 1;transaction < k;transaction++){
            for(int i=1;i<prices.length;i++){
                for(int j=i;j<prices.length;j++){
                    ...
                    int end = j;
                    if(transaction == 1)
                        profits[start][end][1] = maxProfitOnce(prices, start, end);
                    else{
                        if(transaction > end - start)
                            profits[start][end][transaction] = profits[start][end][end-start];
                        else{
                            int subMax = 0;
                            for(int split=start+1;split<end;split++){
                                for(int part=1;part<transaction;part++){
                                    int sub = profits[start][split][part]+profits[split][end][transaction-part];
                                    if(sub > subMax)
                                        subMax = sub;
                                }
                            }
                            profits[start][end][transaction] = subMax;
                        }
                    }
                }
            }
        ...
        for(int split=1;split<prices.length;split++){
            for(int part=1;part<k;part++){
                int sub = profits[0][split][part]+profits[split][prices.length-1][k-part];
                if(sub > result)
                    result = sub;
            }
        }
        for(int transaction = 1;transaction < k;transaction++){
            if(profits[0][prices.length-1][transaction] > result)
                result = profits[0][prices.length-1][transaction];
        }
        return result;
    }

    private int maxProfitAny(int[] prices) {
        int profit=0;
    ...
    private int maxTransactions(int[] prices) {
        int transactions = 0;
        boolean firstGrow = true;
        int prev = prices[0];
        for(int i=1;i<prices.length;i++){
            if(prices[i] > prev && firstGrow){
                ++transactions;
                firstGrow = false;
            }
    ...
    public static void main(String[] args) {
        Number188 number188 = new Number188();
        ...
        System.out.println(number188.maxProfit(2, prices));
    }
}
