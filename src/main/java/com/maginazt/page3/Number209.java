...
//        if(sum < s)
//            return 0;
//        int minimalLength = right - left + 1;
...
//    }
    ...
    public int minSubArrayLen(int s, int[] nums) {
        if(nums == null || nums.length == 0)
            return 0;
        int result = minSubArrayLen(s, nums, 0, nums.length-1);
        return result == Integer.MAX_VALUE ? 0 : result;
    }

    private int minSubArrayLen(int s, int[] nums, int start, int end) {
        if(start == end)
            return nums[start] >= s ? 1 : Integer.MAX_VALUE;
        int mid = (end+start)/2;
        int minLen = Math.min(minSubArrayLen(s, nums, start, mid), minSubArrayLen(s, nums, mid+1, end));
        int turnLeft = mid;
        int turnRight = mid + 1;
        int sum = nums[turnLeft] + nums[turnRight];
        while (sum < s && turnLeft > start && turnRight < end){
            if(nums[turnLeft-1] > nums[turnRight+1])
                sum += nums[--turnLeft];
            else
                sum += nums[++turnRight];
        }
        if(sum >= s){
            minLen = Math.min(minLen, turnRight-turnLeft+1);
        }
        while (sum < s && turnLeft > start){
            sum += nums[--turnLeft];
        }
        if(sum >= s){
            minLen = Math.min(minLen, turnRight-turnLeft+1);
        }
        while (sum < s && turnRight < end)
            sum += nums[++turnRight];
        if(sum >= s){
            minLen = Math.min(minLen, turnRight-turnLeft+1);
        }
        System.out.println(start+","+end+" : "+minLen);
        return minLen;
    }
